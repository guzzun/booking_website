$(document).ready(function()
{
    $('.open-button').click(function() 
    {
            if(!$('.menu-button').hasClass('open-done'))
            {
                ...
                setTimeout(function(){$('.menu-link-5').addClass('open-done')}, 850);
            }
            else
            {
                $('.menu-button').removeClass('open-done');
                $('.menu-link-1').removeClass('open-done');
                // $('.active_nav').removeClass('open-done');
                setTimeout(function(){$('.active_nav').removeClass('open-done')}, 800);
